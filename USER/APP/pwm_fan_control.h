 ...
 */
typedef enum {
    FAN_MODE_NORMAL = 0,   ///< 普通模式：固定追踪用户设定的基础风速
    FAN_MODE_SMART,        ///< 智能风模式：根据当前环境温度动态调整基础风速
    FAN_MODE_NATURAL       ///< 自然风模式：在基础风速上叠加气象、湍流、阵风三大引擎
} FanMode_t;
 ...
 */
typedef struct {
    FanConfig_t config;     ///< 风扇算法配置参数
    
    FanMode_t mode;         ///< 当前风扇运行模式
 ...
 * @param fan 指向 FanController_t 结构体的指针
 */
